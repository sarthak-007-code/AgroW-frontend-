...
(async () => {
    try {
        ...
        // Fetch comments via POST contentId
        const commentsBody = JSON.stringify({ contentId });
        const resCommentsPost = await request('https://braided-constrained-maximilian.ngrok-free.dev/api/content/getContentComments', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Content-Length': commentsBody.length, 'ngrok-skip-browser-warning': 'true' }
        }, commentsBody);

        log += "POST /getContentComments (body: contentId) Status: " + resCommentsPost.status + "\n";
        log += "Response: " + resCommentsPost.data.substring(0, 500) + "\n";

        const commentsBodypostId = JSON.stringify({ postId: contentId });
        const resCommentsPost2 = await request('https://braided-constrained-maximilian.ngrok-free.dev/api/content/getContentComments', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Content-Length': commentsBodypostId.length, 'ngrok-skip-browser-warning': 'true' }
        }, commentsBodypostId);

        log += "POST /getContentComments (body: postId) Status: " + resCommentsPost2.status + "\n";
        log += "Response: " + resCommentsPost2.data.substring(0, 500) + "\n";
        ...
        log += "Response: " + resCommentsGet.data.substring(0, 500) + "\n";

        fs.writeFileSync('result.txt', log);
    } catch (err) {
